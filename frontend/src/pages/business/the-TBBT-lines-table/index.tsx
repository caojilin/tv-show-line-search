...
import { Button, Col, Form, Input, InputNumber, List, Row, Select, Space, Table, theme, TreeSelect, Typography } from 'antd';
...
const App: React.FC = () => {
  const default_tableData = data.filter((item: { season: number; episode: number; }) => item.season == 1 && item.episode == 1);
  const [tableData, setTableData] = useState(default_tableData);
  ...
  return (
    <div>
      <>
        <Row>
          <Col span={24}>
            <TreeSelect defaultValue="1-1" style={{ width: 720 }} treeData={season_episode} onChange={onChange} />
          </Col>
        </Row>
        <Row>
          <Col span={24}>
            ...
            <List
              ...
              renderItem={(item: any) => (
                <List.Item>
                  <Typography.Text strong>{item.character}:</Typography.Text> {item.text}
                </List.Item>
              )}
            />
          </Col>
